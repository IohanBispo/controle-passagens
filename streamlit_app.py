...
# 2. Adicionar Recarga com DATA MANUAL
st.sidebar.subheader("➕ Adicionar Recarga")
data_recarga = st.sidebar.date_input("Data da Recarga", datetime.now())
...
if st.sidebar.button("Confirmar Recarga"):
    ...
    st.session_state.historico.append({
        "Data": data_recarga.strftime("%d/%m/%Y"),
        "Tipo": "Recarga",
        "Valor": f"R$ {valor_recarga:.2f}",
        "Saldo Resultante": f"R$ {st.session_state.saldo:.2f}"
    })
    st.sidebar.success("Recarga registrada no histórico!")
    st.rerun()
...
if st.button("Confirmar Uso"):
    ...
    if st.session_state.saldo >= custo_total:
        ...
        st.success(f"Uso registrado!")
        st.rerun()
    else:
        st.error("Saldo insuficiente!")

# --- 5. HISTÓRICOS (CORRIGIDOS) ---
st.divider()
...
if st.session_state.historico:
    ...
    with aba_uso: st.info("Histórico vazio.")
    with aba_recarga: st.info("Histórico vazio.")
